...
pub enum SysxError {
    ...
    /// Mutex/RwLock poisoned error.
    #[error("Mutex/RwLock poisoned: {0}")]
    SyncPoison(#[from] std::sync::PoisonError<Box<dyn std::any::Any + Send>>),
}
    ...
    for SysxError 
{
    fn from(err: std::sync::PoisonError<std::sync::MutexGuard<'_, std::collections::HashMap<String, String>>>) -> Self {
        SysxError::SyncPoison(format!("Mutex consistency error: {}", err))
    }
}
